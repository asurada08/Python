...
print('# index 속성을 이용하여 색인 전체를 구할 수 있다')
print(members.index)
...
for idx in range(members.size):
    value = str(members.iloc[idx]) + '점'  # 예시 : 60건
    ratioval = '%.1f%%' % (ratio.iloc[idx])  # 예시 : 20.0%
    # 그래프 위에 '건수' 표시
    plt.text(x=idx, y=members.iloc[idx]+1, s=value, horizontalalignment='center')
    # 그래프 중간에 비율 표시
    plt.text(x=idx, y=members.iloc[idx]/2, s=ratioval, horizontalalignment='center')
